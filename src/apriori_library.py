...
class AssociationRulesMiner:
    ...
    def calculate_weighted_metrics(self, weights_series: pd.Series, basket_bool: pd.DataFrame = None):
        """
        Tính toán Weighted Support cho các luật đã sinh ra.
        
        Args:
            weights_series (pd.Series): Series với index là InvoiceNo và value là TotalPrice (trọng số).
            basket_bool (pd.DataFrame): Ma trận basket boolean (nếu None sẽ dùng self.basket_bool).
        
        Returns:
            pd.DataFrame: DataFrame luật kèm cột weighted_support.
        """
        if self.rules is None:
            raise ValueError("Chưa có luật nào được sinh ra. Hãy chạy generate_rules() trước.")
        
        if basket_bool is None:
            basket_bool = self.basket_bool

        # Đảm bảo index của weights và basket_bool khớp nhau (đều là InvoiceNo)
        # Chỉ lấy những hóa đơn có trong cả 2 bên
        common_invoices = basket_bool.index.intersection(weights_series.index)
        basket_subset = basket_bool.loc[common_invoices]
        weights_subset = weights_series.loc[common_invoices]
        
        total_weight = weights_subset.sum()
        ws_list = []
        
        print("Đang tính toán Weighted Support... (có thể mất chút thời gian)")
        
        # Duyệt qua từng luật để tính
        for _, row in self.rules.iterrows():
            # Lấy tất cả sản phẩm trong luật (Antecedents U Consequents)
            items = list(row['antecedents']) + list(row['consequents'])
            
            # Tìm các hóa đơn chứa tất cả sản phẩm này (phép AND)
            # Kiểm tra xem các cột items có tồn tại trong basket không để tránh lỗi
            valid_items = [i for i in items if i in basket_subset.columns]
            
            if len(valid_items) < len(items):
                ws_list.append(0)
                continue

            # Mask: Các dòng (hóa đơn) có chứa toàn bộ item trong luật
            mask = basket_subset[valid_items].all(axis=1)
            
            # Tổng giá trị các hóa đơn thỏa mãn / Tổng giá trị toàn bộ
            weighted_sup = weights_subset[mask].sum() / total_weight
            ws_list.append(weighted_sup)
            
        self.rules['weighted_support'] = ws_list
        
        # Tính Weighted Lift (xấp xỉ = Weighted Support / (Support Ant * Support Cons))
        # Lưu ý: Đây là công thức ước lượng để so sánh tương quan
        self.rules['weighted_lift_proxy'] = self.rules['weighted_support'] / (self.rules['antecedent support'] * self.rules['consequent support'])
